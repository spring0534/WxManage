...
class Image{
	 ...
	 * @方法名：watermark
	 * @param  $source 原文件
	 * @param  $target 加水印后的文件名，传空值则用原名
	 * @param  $config 配置
	 * @author 	佚名
	 * @2012-11-16下午11:37:08
	 */
	static public  function watermark($source,$target = '',$config ) {
		...
		$fontface =  './Public/font/'.$config['watemard_text_face'];
		$w_pct =   $config['watermark_pct'];
		$w_quality =  $config['watermark_quality'];
		$w_minheight =  $config['watermark_minwidth'];
		$w_minwidth =  $config['watermark_minheight'];
		$watermarkPadding = $config['watermark_pospadding'];
		$w_pos = $config['watermark_pos'];
		$positionPadding = ($watermarkPadding && $watermarkPadding > 0) ? $watermarkPadding : 5; 	// 边距
		$w_pos = $w_pos ? $w_pos : 9;
		$w_img = $w_img ? $w_img : './Public/images/watermark.png';
    ...
    static function buildImageVerify($length=4, $mode=1, $type='png', $width=48, $height=22, $verifyName='verify') {
       ...
       for ($i = 0; $i < $length; $i++) {
		  ...
		  imagettftext($im,30,0, $i * 20 + 30,mt_rand(30, 40), $stringColor , dirname(__FILE__).'/t1.ttf' ,$randval{$i});
