...
class Common_util_pub{
	...
	public function getSign($Obj){
		...
		//签名步骤二：在string后加入KEY
		$String = $String."&key=".$GLOBALS['accountinfo']['partnerkey'];
		//签名步骤三：MD5加密
		$String = md5($String);
		//签名步骤四：所有字符转为大写
	...
	function postXmlSSLCurl($xml,$url,$second=30){
		...
		curl_setopt($ch,CURLOPT_SSLCERT, $GLOBALS['accountinfo']['sslcertPath']);
		//默认格式为PEM，可以注释
		curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
		curl_setopt($ch,CURLOPT_SSLKEY, $GLOBALS['accountinfo']['sslkeyPath']);
		//post提交方式
		curl_setopt($ch,CURLOPT_POST, true);
		curl_setopt($ch,CURLOPT_POSTFIELDS,$xml);
		$data = curl_exec($ch);
...
class Wxpay_client_pub extends Common_util_pub {
	 ...
	 */
	function createXml(){
	   	$this->parameters["appid"] = $GLOBALS['accountinfo']['appid'];//公众账号ID
	   	$this->parameters["mch_id"] = $GLOBALS['accountinfo']['mchid'];//商户号
...
class UnifiedOrder_pub extends Wxpay_client_pub{	
	function __construct() {
		...
		//设置curl超时时间
		$this->curl_timeout = $GLOBALS['accountinfo']['curlTimeout'];
	}
...
class NativeCall_pub extends Wxpay_server_pub{
	...
	function createXml(){
		if($this->returnParameters["return_code"] == "SUCCESS"){
		   	$this->returnParameters["appid"] = $GLOBALS['accountinfo']['appid'];//公众账号ID
		   	$this->returnParameters["mch_id"] = $GLOBALS['accountinfo']['mchid'];//商户号
...
class JsApi_pub extends Common_util_pub{
	...
	function createOauthUrlForCode($redirectUrl){
		$urlObj["appid"] = $GLOBALS['accountinfo']['appid'];
	...
	function createOauthUrlForOpenid(){
		$urlObj["appid"] = $GLOBALS['accountinfo']['appid'];
		$urlObj["secret"] = $GLOBALS['accountinfo']['appsecret'];
	...
	public function getParameters(){
		$jsApiObj["appId"] = $GLOBALS['accountinfo']['appid'];
		$timeStamp = time();
