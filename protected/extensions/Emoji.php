...
class Emoji {
	 ...
	 * @return
	 */
	public function fileterEmoji($source) {
		$clean_text = $this->emoji_docomo_to_unified($source);
		$html =  $this->emoji_unified_to_html($clean_text);
		$clean_text = $this->emoji_kddi_to_unified($html);
		$html =  $this->emoji_unified_to_html($clean_text);
		$clean_text = $this->emoji_softbank_to_unified($html);
		$html =  $this->emoji_unified_to_html($clean_text);
		$clean_text = $this->emoji_google_to_unified($html);
		$html =  $this->emoji_unified_to_html($clean_text);
		return strip_tags( $html);

	}
	public function fileterEmojitest() {
		$s = "💞☺😳👂😰😠🙏💃💂👷👴👳👲😌😋🈵<body>😄213这是一个有各种内容的消息,  Hia Hia Hia !!!! xxxx@@@...*)!(@*$&@(&#!)@*)!&$!)@^%@(!&#. 😄👩👨], ";
		$clean_text = $this->emoji_google_to_unified($s);
		dump($clean_text);
		$html =  $this->emoji_unified_to_html($clean_text);
		dump(strip_tags( $html));
