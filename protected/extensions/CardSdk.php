...
class BaseInfo{
	function __construct($logo_url, $brand_name, $code_type, $title, $color, $notice, $service_phone, $description, $date_info, $sku){
		if (!$date_infoinstanceofDateInfo)
...
class Card{ // 工厂
            // DISCOUNT(折扣券), CASH(现金券), GENERAL_COUPON(普通优惠券), GIFT(礼品券), GROUPON(团购券), LUCK_MONEY(红包)
	...
	function __construct($card_type, $base_info){
		if (!in_array($card_type, $this->CARD_TYPE))
			exit("CardType Error");
		if (!$base_infoinstanceofBaseInfo)
...
class CardSdk{
	...
	public function getSignPackage($ghid, $card_id, $code='', $openid='', $balance=''){
		$nonceStr=$this->createNonceStr();
		$timestamp=time();
		$row=Yii::app()->db->createCommand()
		->select('appid,appsecret,access_token,at_expires,jsapi_ticket,jsapi_ticket_expire,jsapi,dev_oauth_ghid,dev_jsapi_ghid,cardapi_ticket,cardapi_ticket_expire')
		->from('sys_user_gh')
		...
		$account=array(
			...
			'cardapi_ticket'=>$row['cardapi_ticket'],
			'cardapi_ticket_expire'=>$row['cardapi_ticket_expire']
		);
		...
		$t->debug=true;
		$cardTicket=$t->getCardTicket();
		$signature=new Signature();
		$signature->add_data($cardTicket);
	...
	function getcardSign($ghid, $card_id, $card_type, $location_id=''){
		$nonceStr=$this->createNonceStr();
		$timestamp=time();
		$row=Yii::app()->db->createCommand()
		->select('appid,appsecret,access_token,at_expires,jsapi_ticket,jsapi_ticket_expire,jsapi,dev_oauth_ghid,dev_jsapi_ghid,cardapi_ticket,cardapi_ticket_expire')
		...
		return array(
			'cardSign'=>$signature->get_signature(),
			'timestamp'=>$timestamp,
			'nonceStr'=>$nonceStr,
			'card_id'=>$card_id,
			'card_type'=>$card_type,
