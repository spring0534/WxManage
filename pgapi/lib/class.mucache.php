<?php
/**
 * memcacheq消息队列只支持set,get,delete方法,单次设置不能超过64k(可以压缩)
 * memcachedb（新浪互动社区技术团队开发）不能使用CAS协议
 * CAS协议不能同时使用分组备份机制
 * 对于高精度数据,不要用分布式
...
class mucache{
	...
	public function __construct( $arrServers, $persistent=false){ 
	...
	private function errorlog($keys, $try, $group, $msg , $method){
		...
		$file = dirname(__FILE__) . '/../data/memcache.txt';
