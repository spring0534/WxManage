...
(function(b) {
    b.fn.dragsort = function(o) {
        ...
        this.selector && b("head").append("<style type='text/css'>" + (this.selector.split(",").join(" " + e.dragSelector + ",") + " " + e.dragSelector) + " { cursor: move; }</style>");
        this.each(function(p, k) {
            if (b(k).is("table") && b(k).children().size() == 1 && b(k).children().is("tbody"))
                k = b(k).children().get(0);
            var n = {draggedItem: null,placeHolderItem: null,pos: null,offset: null,offsetLimit: null,scroll: null,container: k,init: function() {
                    b(this.container).attr("data-listIdx", p).mousedown(this.grabItem).find(e.dragSelector).css("cursor", "move");
                    ...
                    if (!(d.which != 1 || b(d.target).is(e.dragSelectorExclude))) {
                        ...
                        if (e.itemSelector == "tr") {
                            ...
                            a.placeHolderItem = a.draggedItem.next().css({height: c,width: f}).attr("data-placeHolder", true)
                        }
                        ...
                        a.scroll.scrollY = window.setInterval(function() {
                            if (e.scrollContainer != window)
                                b(e.scrollContainer).scrollTop(b(e.scrollContainer).scrollTop() + a.scroll.moveY);
                            else {
                        ...
                        a.scroll.scrollX = window.setInterval(function() {
                            if (e.scrollContainer != window)
                                b(e.scrollContainer).scrollLeft(b(e.scrollContainer).scrollLeft() + a.scroll.moveX);
                            else {
                    ...
                    this.draggedItem.parents().each(function() {
                        if (b(this).css("position") != "static" && (!b.support.mozilla || b(this).css("display") != "table")) {
                            var m = b(this).offset();
                            f -= m.top;
                            h -= m.left;
                            return false
                    ...
                    if ((b.support.safari || b.support.mozilla) && a && e.scrollContainer != window) {
                    ...
                    if (a.draggedItem != null) {
                        b(a.container).find(e.dragSelector).css("cursor", "move");
                    ...
                    for (var f = 0; f < this.pos.length; f++)
                        if (this.pos[f].left < d && this.pos[f].right > d && this.pos[f].top < c && this.pos[f].bottom > c)
                            return f;
                    return -1
                    ...
                    e.dragBetween && b(j).each(function() {
                        ...
                        if (d.size() > 0 && c.size() > 0)
                            ...
                            d.size() == 0 && c.size() == 0 && b(this.container).append(a.placeHolderItem.clone().removeAttr("data-placeHolder").attr("data-dropTarget", true))
                    })
                }};
            n.init();
            j.push(n)
    ...
    b.fn.dragsort.defaults = {itemSelector: "li",dragSelector: "li",dragSelectorExclude: "input, textarea, a[href]",dragEnd: function() {
        },dragBetween: false,placeHolderTemplate: "<li>&nbsp;</li>",scrollContainer: window,scrollSpeed: 5}
})(jQuery);
