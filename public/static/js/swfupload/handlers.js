...
function att_show(serverData,file)
{
	...
	var img = "";
	if(ext == 1) {
	    img = '<div class="get selected" id="aid-'+id+'"><a class="del" href="javascript:;"></a> <img onclick="att_cancel(this,'+id+',\'upload\')" width="87" height="98" src="'+src+'" data-id="'+id+'" data-path="'+src+'" alt="上传完成" title="'+filename+'"><input type="text" class="J_file_desc" name="flashatt['+id+'][desc]" placeholder="请输入描述" value="'+filename+'" style="width:68px"></div>';
	} else {
		img = '<div class="get selected" id="aid-'+id+'"><a class="del" href="javascript:;"></a> <img onclick="att_cancel(this,'+id+',\'upload\')" width="87" height="98" src="'+WEBURL+'/static/images/ext/'+ext+'.gif" data-id="'+id+'" data-path="'+src+'" alt="上传完成" title="'+filename+'"><input type="text" class="J_file_desc" name="flashatt['+id+'][desc]" placeholder="请输入描述" value="'+filename+'"  style="width:68px"></div>';
	}
...
function uploadError(file, errorCode, message) {
	...
	switch (errorCode)
	{
		...
		case SWFUpload.UPLOAD_ERROR.UPLOAD_LIMIT_EXCEEDED:
			msg = '单次上传文件数限制为 '+swfu_Contents.settings.file_upload_limit+' 个';
			break;
		default:
			msg = message;
			break;
...
function fileQueueError(file, errorCode, message)
{
	...
	case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
		...
		errormsg = '单次上传文件数限制为 '+swfu_Contents.settings.file_upload_limit+' 个';
		break;
